...
const QuickSort = require('../core').QuickSort;
const qs = new QuickSort();

let testArray1 = [3, 5, 7, 4, 2, 1, 0];
let testArray2 = [1, 11, 6, 2, 4, 5, 3];
let testArray_test = [1, 11, 6, 2, 4, 5, 3];
let testArrayLong = require('./longarray').testArrayLong;
...
assert.strictEqual(
  JSON.stringify(qs.sort(testArray3, { isClone: true, item: 'value' })),
  JSON.stringify([{ name: 'first', value: 1},{ name: 'second', value: 2},{ name: 'third', value: 3},{ name: 'fourth', value: 4},{ name: 'fifth', value: 5},{ name: 'sixth', value: 6},{ name: 'seventh', value: 7},{ name: 'eight', value: 8},{ name: 'ninth', value: 9},{ name: 'tenth', value: 10}])
)

console.log("#. `QuickSort` isEqual test on long array");
console.log(testArrayLong.length);
console.time("QuickSort");
qs.sort(testArrayLong, { isClone: false })
